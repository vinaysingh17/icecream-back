...
const UomSchema = require("../Schema/Uom.schema");
...
const create = async (req, res, next) => {
  ...
  try {
    ...
    const savedData = await UomSchema.create({
      ...req.body,
    });
...
const read = async (req, res, next) => {
  try {
    const data = await UomSchema.find(req.query);
...
const Delete = async (req, res, next) => {
  try {
    const { id } = req.params;
    const data = await UomSchema.findByIdAndDelete(id);
